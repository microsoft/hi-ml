...
def test_get_biovil_t_linear_image_classifier_shape(
    biovil_t_linear_image_classifier: ImageModel, dummy_input_tensor: torch.Tensor
) -> None:
    num_classes = 2
    num_tasks = 8
...
def test_get_biovil_t_linear_image_classifier_inference(
    biovil_t_linear_image_classifier: ImageModel, dummy_input_tensor: torch.Tensor
) -> None:
    with torch.no_grad():
        try:
