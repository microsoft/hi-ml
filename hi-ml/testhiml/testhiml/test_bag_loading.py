...
from health_ml.utils.histopathology.bag_utils import (BagSampler, create_bag_dataloader, multibag_collate)
...
def _test_lightning_tiles_dataset(base_dataset: TilesDataset) -> None:
    # The mock dataset tests already check the behaviour across shuffling and GPU settings,
    # so here we do a single epoch on CPU, fully shuffled, as the real datasets take much longer
    dataset = Dataset(base_dataset, transform=LoadTiled(base_dataset.IMAGE_COLUMN))  # type: ignore
    module = InstrumentedLightningModule(
        dataset,  # type: ignore
        bag_ids=base_dataset.dataset_df.slide_id,
        batch_size=10,
        shuffle_bags=True,
        shuffle_samples=True,
        get_instance_id=lambda batch: batch[base_dataset.TILE_ID_COLUMN],
        get_bag_id=lambda batch: batch[base_dataset.SLIDE_ID_COLUMN],
        seed=0,
        num_workers=os.cpu_count())
...
def test_lightning_tcga_crck_tiles(train: bool) -> None:
    _test_lightning_tiles_dataset(
        TcgaCrck_TilesDataset(TCGA_CRCK_DATASET_DIR, train=train)
    )


@pytest.mark.skipif(not os.path.isdir(PANDA_TILES_DATASET_DIR),
                    reason="PANDA tiles dataset is unavailable")
def test_lightning_panda_tiles() -> None:
    _test_lightning_tiles_dataset(
        PandaTilesDataset(PANDA_TILES_DATASET_DIR)
    )
