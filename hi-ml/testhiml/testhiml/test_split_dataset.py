...
def test_split_by_subject_ids() -> None:
    ...
    splits = DatasetSplits.from_subject_ids(
        test_df, train_ids, test_ids, val_ids, subject_column=CSV_SUBJECT_HEADER)
...
def test_split_by_subject_ids_invalid(splits: List[List[str]]) -> None:
    df1 = pd.read_csv(full_test_data_path(
        suffix=DATASET_CSV_FILE_NAME), dtype=str)
...
def test_grouped_splits(group_column: str) -> None:
    ...
    _check_is_partition(
        test_df, [splits.train, splits.test, splits.val], CSV_SUBJECT_HEADER)
    _check_is_partition(
...
def _get_test_df() -> Tuple[DataFrame, List[str], List[str], List[str]]:
    ...
    assert all(np.bincount(test_data[CSV_GROUP_HEADER])
               > 1), "Found singleton groups"  # type: ignore
    assert len(np.unique(test_data[CSV_GROUP_HEADER])
               ) > 1, "Found a single group"  # type: ignore
    train_ids, test_ids, val_ids = list(range(0, 50)), list(
        range(50, 75)), list(range(75, 100))
