...
from health_ml.utils.data_augmentations import (
    HEDJitter,
    StainNormalization,
    GaussianBlur,
...
dummy_img = torch.Tensor(
    [[[[0.4767, 0.0415], [0.8325, 0.8420]], [[0.9859, 0.9119], [0.8717, 0.9098]], [[0.1592, 0.7216], [0.8305, 0.1127]]]]
)
dummy_bag = torch.stack([dummy_img.squeeze(0), dummy_img.squeeze(0)])


def _test_data_augmentation(
    data_augmentation: Callable[[Tensor], Tensor],
...
def test_hed_jitter() -> None:
    ...
    expected_output_img = torch.Tensor(
        [
            [
                [[0.3375, 0.0000], [0.6677, 0.3245]],
                [[0.2473, 0.0031], [1.0000, 0.1836]],
                [[0.0877, 0.1219], [0.6196, 0.0418]],
            ]
        ]
    )
    ...
    _test_data_augmentation(
        data_augmentation, dummy_img.squeeze(0), expected_output_img.squeeze(0), stochastic=True, seed=1, atol=1e-3
    )
...
def test_gaussian_blur() -> None:
    ...
    expected_output_img = torch.Tensor(
        [
            [
                [[0.5953, 0.6321], [0.4528, 0.5125]],
                [[0.9126, 0.9138], [0.9229, 0.9299]],
                [[0.4343, 0.4848], [0.4601, 0.4448]],
            ]
        ]
    )
...
def test_random_rotation() -> None:
    ...
    expected_output_img = torch.Tensor(
        [
            [
                [[0.0415, 0.8420], [0.4767, 0.8325]],
                [[0.9119, 0.9098], [0.9859, 0.8717]],
                [[0.7216, 0.1127], [0.1592, 0.8305]],
            ]
        ]
    )
