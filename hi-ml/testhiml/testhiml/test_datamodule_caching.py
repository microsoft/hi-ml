...
def create_mock_dataset_csv() -> None:
    csv_path = MOCK_TILES_DATASET_DIR / MockTilesDataset.DEFAULT_CSV_FILENAME
...
def _get_datamodule(cache_mode: CacheMode, save_precache: bool,
                    cache_dir_provided: bool) -> TilesDataModule:
    ...
    root_path = MOCK_TILES_DATASET_DIR
    cache_dir = root_path / f"datamodule_cache_{cache_mode.value}" \
        if cache_dir_provided else None

    if cache_dir is not None and cache_dir.exists():
        shutil.rmtree(cache_dir)

    return MockTilesDataModule(root_path=root_path,
...
def test_caching_consistency(cache_mode: CacheMode, save_precache: bool,
...
@pytest.mark.parametrize('save_precache', [True, False])
@pytest.mark.parametrize('cache_dir_provided', [True, False])
def test_tile_id_coverage(cache_mode: CacheMode, save_precache: bool,
                          cache_dir_provided: bool) -> None:
    datamodule = _get_datamodule(cache_mode=cache_mode,
                                 save_precache=save_precache,
                                 cache_dir_provided=cache_dir_provided)
