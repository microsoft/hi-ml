...
from health_azure.utils import (
    RUN_CONTEXT, download_files_from_run_id, get_run_file_names, is_running_in_azure_ml)
from health_ml.utils.common_utils import (
    AUTOSAVE_CHECKPOINT_CANDIDATES, CHECKPOINT_FOLDER, DEFAULT_AML_UPLOAD_DIR)
...
def download_folder_from_run_to_temp_folder(folder: str,
    ...
    if not is_running_in_azure_ml() and run is None:
        raise ValueError(
            "When running outside AzureML, the run to download from must be set.")
    ...
    logging.info(
        f"Number of checkpoints available in AzureML: {len(existing_checkpoints)}")
    if len(existing_checkpoints) > 0:
        try:
            ...
            logging.warning(
                f"Unable to download checkpoints from AzureML. Error: {str(ex)}")
...
def get_recovery_checkpoint_path(path: Path) -> Path:
    ...
    if recovery_checkpoint is None:
        ...
        raise FileNotFoundError(
            f"No checkpoint files found in {path}. Existing files: {' '.join(files)}")
    return recovery_checkpoint
...
def find_recovery_checkpoint(path: Path) -> Optional[Path]:
    ...
    """
    legacy_recovery_checkpoints = list(
        path.glob(LEGACY_RECOVERY_CHECKPOINT_FILE_NAME + "*"))
    if len(legacy_recovery_checkpoints) > 0:
        logging.warning(
            f"Found these legacy checkpoint files: {legacy_recovery_checkpoints}")
    ...
    candidates = [*AUTOSAVE_CHECKPOINT_CANDIDATES,
        ...
        if full_path.is_file():
            try:
                checkpoint = torch.load(
                    str(full_path), map_location=torch.device("cpu"))
                ...
                logging.warning(
                    f"Unable to load checkpoint file {full_path}: {ex}")
...
def cleanup_checkpoints(ckpt_folder: Path) -> None:
    ...
    if len(files_in_checkpoint_folder) == 0:
        return
    logging.info(
        f"Files in checkpoint folder: {' '.join(files_in_checkpoint_folder)}")
    ...
    if not last_ckpt.is_file():
        raise FileNotFoundError(
            f"Checkpoint file {LAST_CHECKPOINT_FILE_NAME_WITH_SUFFIX} not found. {all_files}")
