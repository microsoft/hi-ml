...
class DeepSMILESlidesPandaBenchmark(DeepSMILESlidesPanda):
    ...
    def get_transforms_dict(self, image_key: str) -> Dict[ModelKey, Union[Callable, None]]:
        ...
        # Make inference transforms deterministic (not needed right now, just in case we change that later)
        transform_inf.set_random_state(seed=self.get_effective_random_seed())
