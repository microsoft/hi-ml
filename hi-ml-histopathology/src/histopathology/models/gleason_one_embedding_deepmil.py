...
class DeepMILClassifier(nn.Module):
    ...
    def get_classifier(self) -> Callable:
        classifier_layer = nn.Linear(in_features=self.num_features,
    ...
    def forward(self, bag_features):
        return self.classifier_fn(bag_features)


class GleasonOneEmbeddingDeepMILModule(LightningModule):
    """Base class for deep multiple-instance learning"""

    def __init__(self,
                 label_column: List[str, str],
                 n_classes: int,
                 encoder: TileEncoder,
        ...
        self.primary_label_column = label_column[0]
        self.secondary_label_column = label_column[1]
        self.n_classes = n_classes
        ...
        # Create classifiers
        self.primary_classifier = DeepMILClassifier('PRIMARY',
                                     n_classes,
                                     num_features,
                                     dropout_rate,
                                     class_names,
        ...
        self.secondary_classifier = DeepMILClassifier('SECONDARY',
                                     n_classes,
                                     num_features,
                                     dropout_rate,
                                     class_names,
    ...
    def forward(self, instances: Tensor) -> Tuple[Tensor, Tensor]:  # type: ignore
        with set_grad_enabled(self.is_finetune):
            instance_encodings = self.encoder(instances)                    # N X L x 1 x 1
        attentions, bag_encoding = self.aggregation_fn(instance_encodings)  # K x N | K x L
        bag_encoding = bag_encoding.view(1, -1)
        primary_bag_logits = self.primary_classifier(bag_encoding)
        secondary_bag_logits = self.secondary_classifier(bag_encoding)

        return primary_bag_logits, secondary_bag_logits, attentions
    ...
    def _shared_step(self, batch: Dict, batch_idx: int, stage: str) -> BatchResultsType:
        ...
        primary_bag_labels_list = []
        primary_bag_logits_list = []
        secondary_bag_labels_list = []
        secondary_bag_logits_list = []
        bag_attn_list = []
        
        for bag_idx in range(len(batch[self.label_column])):
            images = batch[TilesDataset.IMAGE_COLUMN][bag_idx]
            primary_labels = batch[self.primary_label_column][bag_idx]
            secondary_labels = batch[self.secondary_label_column][bag_idx]
            primary_bag_labels_list.append(self.get_bag_label(primary_labels))
            secondary_bag_labels_list.append(self.get_bag_label(secondary_labels))
            primary_logits, secondary_logits, attn = self.forward(images)
            primary_bag_logits_list.append(primary_logits.view(-1))
            secondary_bag_logits_list.append(secondary_logits.view(-1))
            bag_attn_list.append(attn)

        primary_bag_logits = torch.stack(primary_bag_logits_list)
        secondary_bag_logits = torch.stack(secondary_bag_logits_list)
        primary_bag_labels = torch.stack(primary_bag_labels_list).view(-1)
        secondary_bag_labels = torch.stack(secondary_bag_labels_list).view(-1)
        ...
        results = self.primary_classifier.update_result_dict(primary_bag_logits, primary_bag_labels, results, stage)
        results = self.secondary_classifier.update_result_dict(secondary_bag_logits, secondary_bag_labels, results, stage)
    ...
    def training_step(self, batch: Dict, batch_idx: int) -> Tensor:  # type: ignore
        ...
        self.classifier.log_metrics('train', self)
        return train_result[ResultsKey.LOSS] + second loss
    ...
    def validation_epoch_end(self, epoch_results: EpochResultsType) -> None:  # type: ignore
        if self.outputs_handler:
            self.outputs_handler.save_validation_outputs(
                ...
                metrics_dict=self.classifier.get_metrics_dict('val'),
