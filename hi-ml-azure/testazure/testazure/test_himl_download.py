...
def test_download_aml_run_args(tmp_path: Path) -> None:
    ...
    with pytest.raises(Exception) as e:
        subprocess.Popen(["python", DOWNLOAD_SCRIPT_PATH,
...
def test_no_config_path(tmp_path: Path) -> None:
    ...
    with pytest.raises(Exception) as e:
        subprocess.Popen(["python", DOWNLOAD_SCRIPT_PATH, "--config_path",
                         "idontexist", "--output_dir", str(tmp_path)])
        assert "You must provide a config.json file in the root folder to connect" in str(
            e)
...
def test_download_aml_run_no_runs(tmp_path: Path) -> None:
    ...
    with pytest.raises(Exception) as e:
        subprocess.Popen(["python", DOWNLOAD_SCRIPT_PATH, "--run_id",
                         "madeuprun", "--output_dir", str(tmp_path)])
...
def test_retrieve_runs() -> None:
    with patch("health_azure.utils.get_aml_run_from_run_id") as mock_get_run:
        ...
        dummy_download_config = himl_download.HimlDownloadConfig(run=[
                                                                 dummy_run_id])
