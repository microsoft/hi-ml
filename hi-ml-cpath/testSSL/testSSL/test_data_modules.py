...
def test_weights_module() -> None:
    ...
    # Assert we have two images and one label given the CIFARTrainTransform
...
def test_vision_module() -> None:
    ...
    # Assert we have one image and one label given the CIFARLinearHeadTransform
...
def test_vision_datamodule_with_return_index() -> None:
    ...
    data_module.setup()
    training_batch = next(iter(data_module.train_dataloader()))
    # Assert we have one one index, one image and one label given the CIFARLinearHeadTransform
