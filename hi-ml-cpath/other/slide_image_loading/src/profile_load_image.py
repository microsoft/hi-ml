...
def profile_cucim(input_file: Path, output_file: Path) -> None:
    """
    ...
    region = img.read_region(location=(0, 0), size=dimensions[count - 1], level=count - 1)
    np_img_arr = np.asarray(region)
    img2 = Image.fromarray(np_img_arr)
    img2.save(output_file)


def profile_openslide(input_file: Path, output_file: Path) -> None:
    """
    ...
    with OpenSlide(str(input_file)) as img:
        ...
        region = img.read_region(location=(0, 0), level=count - 1, size=dimensions[count - 1])
        region.save(output_file)


def profile_folder(mount_point: Path, output_folder: Path, subfolder: str) -> None:
    """
...
def profile_folders(mount_point: Path, output_folder: Path) -> None:
...
def print_cache_state(cache) -> None:  # type: ignore
    ...
    print(
        f"items in cache: {cache.size}/{cache.capacity}, "
        f"memory usage in cache: {cache.memory_size}/{cache.memory_capacity}"
    )


def wrap_profile_folders(mount_point: Path, output_folder: Path) -> None:
    """
...
def profile_main(mount_point: Path, output_folder: Path, label: str, process: Callable) -> None:
    def wrap_main() -> None:
...
def process_slide_open_slide_no_save(
    sample: dict,
    level: int,
    margin: int,
    tile_size: int,
    ...
    return process_slide(
        'openslide', False, sample, level, margin, tile_size, occupancy_threshold, output_dir, tile_progress
    )


def process_slide_cucim_no_save(
    sample: dict,
    level: int,
    margin: int,
    tile_size: int,
    ...
    return process_slide(
        'cucim', False, sample, level, margin, tile_size, occupancy_threshold, output_dir, tile_progress
    )


def process_slide_openslide(
    sample: dict,
    level: int,
    margin: int,
    tile_size: int,
    ...
    process_slide('openslide', True, sample, level, margin, tile_size, occupancy_threshold, output_dir, tile_progress)


def process_slide_cucim(
    sample: dict,
    level: int,
    margin: int,
    tile_size: int,
    ...
    process_slide('cucim', True, sample, level, margin, tile_size, occupancy_threshold, output_dir, tile_progress)
...
def main() -> None:
    ...
    run_info = submit_to_azure_if_needed(
        ...
        input_datasets=[DatasetConfig(name='panda', use_mounting=True, target_folder=panda_folder)],
    ...
    for i, process in enumerate(
        [process_slide_cucim_no_save, process_slide_open_slide_no_save, process_slide_cucim, process_slide_openslide]
    ):
    ...
    for i, process in enumerate([process_slide_cucim_no_save, process_slide_cucim]):
        profile_main(mount_point, output_folder, labels[i], process)
