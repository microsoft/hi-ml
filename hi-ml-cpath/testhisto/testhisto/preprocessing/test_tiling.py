...
from health_cpath.preprocessing.tiling import assemble_tiles_2d, get_1d_padding, pad_for_tiling_2d, tile_array_2d


@pytest.mark.fast
@pytest.mark.parametrize("length,tile_size", [(8, 4), (9, 4), (8, 3), (4, 4), (3, 4)])
...
def test_2d_padding(width: int, height: int, tile_size: int, channels_first: bool) -> None:
    ...
    padded_array, (offset_w, offset_h) = pad_for_tiling_2d(
        input_array, tile_size, channels_first, constant_values=pad_value
    )
...
def _get_2d_meshgrid(width: int, height: int, channels_first: bool = True) -> np.ndarray:
    array = np.stack(np.meshgrid(np.arange(width), np.arange(height)), axis=0 if channels_first else -1)
...
def test_tile_array_2d_both(width: int, height: int, tile_size: int, channels_first: bool) -> None:
    ...
    padded_array, (offset_w, offset_h) = pad_for_tiling_2d(array, tile_size, channels_first, constant_values=0)
...
def test_assemble_tiles_2d(width: int, height: int, tile_size: int, channels_first: bool) -> None:
    array = _get_2d_meshgrid(width, height, channels_first)
    fill_value = 0
    padded_array, padding_offset = pad_for_tiling_2d(array, tile_size, channels_first, constant_values=fill_value)

    tiles, coords = tile_array_2d(array, tile_size, channels_first)

    assembled_array, assembly_offset = assemble_tiles_2d(
        tiles, coords, fill_value=fill_value, channels_first=channels_first
    )
