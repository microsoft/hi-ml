...
class MockTiledWSIDataset(Dataset):
    def __init__(
        self, n_tiles: int, n_slides: int, n_classes: int, tile_size: Sequence[int], random_n_tiles: bool
    ) -> None:
    ...
    def __getitem__(self, index: int) -> List[Dict[SlideKey, Any]]:
        ...
        return [
            {
                SlideKey.SLIDE_ID: f"slide_{self.slide_ids[index]}",
...
def test_image_collate(random_n_tiles: bool) -> None:
    ...
    dataset = MockTiledWSIDataset(
        n_tiles=20, n_slides=10, n_classes=4, tile_size=(1, 4, 4), random_n_tiles=random_n_tiles
    )
